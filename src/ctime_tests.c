...
static void run_tests(secp256k1_context *ctx, unsigned char *key) {
...
#ifdef ENABLE_MODULE_FROST
    {
        ...
        CHECK(secp256k1_frost_compute_pubshare(ctx, &pubshare[0], 2, ids[0], vss_commitment));
        CHECK(secp256k1_frost_compute_pubshare(ctx, &pubshare[1], 2, ids[1], vss_commitment));
        ...
        /* Make sure that previous tests don't undefine msg. It's not used as a secret here. */
        SECP256K1_CHECKMEM_DEFINE(msg, sizeof(msg));
        CHECK(secp256k1_frost_nonce_process(ctx, &session, pubnonce_ptr, 2, msg, ids[0], ids, &cache, &adaptor) == 1);
        ...
        SECP256K1_CHECKMEM_DEFINE(pre_sig, sizeof(pre_sig));
        CHECK(secp256k1_frost_nonce_parity(ctx, &nonce_parity, &session) == 1);
